...
| P7 | PASS | Replaced the obsolete English store screenshot and added dedicated Japanese assets. Each locale has two 1280×800 PNGs: actual current Settings/History Graph captures and an actual generated chart, presented in a captioned layout. Synthetic data is explicitly labelled. Follow-up corrected JPEG bytes mistakenly saved under PNG filenames; PNG signatures, 8-bit RGB/no-alpha encoding, and actual server-rendered thumbnails after reload were verified. See the asset record for the correction. |
