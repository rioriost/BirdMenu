...
App Store upload and processing completed, and build 17 was selected in the 1.2.3 draft on 2026-09-21. English/Japanese store screenshots were replaced and the physical sensor flow was checked with the production-source validation app. Review phone/email fields are intentionally blank per the user’s instruction; existing name fields remain unchanged. No review submission was performed. These stages are tracked separately in the [preflight report](app-store-preflight-1.2.3.md). An archive alone does not establish upload or publication.
